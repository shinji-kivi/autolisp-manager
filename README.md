...
| **コンパイル版対応** | `.lsp` に加え `.fas` / `.vlx` も登録可能（同名の重複は片方だけロード） |
...
2. **「LISP を追加」** ボタンからファイルを選択、またはアプリの画面に直接ドラッグ＆ドロップで LISPファイルを登録
   ...
   - `.lsp`（ソース版）のほか、`.fas` / `.vlx`（コンパイル版）も登録できます
   - コンパイル版は `〇〇.meta` を一緒に渡してもらうとコマンド名とパレットのボタンが表示されます
...
- **[LISP 開発者へ](LISP開発者へ.md)** — パレットに日本語ボタンを表示する方法・テンプレート・コンパイル版（`.fas` / `.vlx`）での配布手順
